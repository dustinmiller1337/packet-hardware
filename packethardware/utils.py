...
def get_smbios_version(prop):
    regex = {
        "smbios_version": re.compile(r"^SMBIOS\s+\ \s+(.*)$", re.MULTILINE),
    }

    if prop not in regex:
        return ""

    smbios_version = cmd_output("dmidecode", "-t", "6")
    print(smbios_version)
    return __re_multiline_first(smbios_version, regex[prop]).strip()
